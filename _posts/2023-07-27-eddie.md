...
To keep track of Eddie’s pose (position and orientation) over time, the node continuously updates the transformation between two frames: the $$ \textcolor{#507D2A}{`/odom`} $$ <mark style="textcolor:green;">`/odom`</mark> frame and the <mark style="textcolor:green;">`/base_footprint`</mark> frame.
